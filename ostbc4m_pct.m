function BER4m = ostbc4m_pct(M, frLen, numPackets, EbNo, idx, seed)
%OSTBC4M_PCT  Orthogonal space-time block coding for 4xM antenna
%configurations.
%
%   BER4M = OSTBC4M_PCT(M, FRLEN, NUMPACKETS, EBNO, IDX, SEED) computes the
%   bit-error rate estimates via simulation for an orthogonal space-time
%   block coded configuration using four transmit antennas and M receive
...
ber_Estimate = zeros(length(EbNo),3);
...
for packetIdx = 1:numPackets
    ...
    txBits = int2bit(data, log2(P));
    rxBits = int2bit(demod4m, log2(P));
    
    % Determine and update BER
    ber_Estimate(idx,:) = errorCalc(txBits, rxBits);
end % End of FOR loop for numPackets

BER4m = ber_Estimate(idx,:);
